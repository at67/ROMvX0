...
namespace Operators
{
    ...
    std::string getVarAsOperand(Expression::Numeric& numeric)
    {
        ...
        switch(numeric._varType)
        {
            ...
            case Expression::IntVar16:
            {
                ...
                {
                    case Expression::Int16Both: operand = "_" + numeric._name;          break;
                    case Expression::Int16Low:  operand = "_" + numeric._name;          break;
                    case Expression::Int16High: operand = "_" + numeric._name + " + 1"; break;
    ...
    void createSingleOp(const std::string& opcode, Expression::Numeric& numeric, const std::string& operand)
    {
        switch(numeric._varType)
        {
            ...
            case Expression::IntVar16:
            {
                switch(numeric._int16Byte)
                {
                    case Expression::Int16Both: Compiler::emitVcpuAsm(opcode, "_" + numeric._name + operand, false);          break;
                    case Expression::Int16Low:  Compiler::emitVcpuAsm(opcode, "_" + numeric._name + operand, false);          break;
                    case Expression::Int16High: Compiler::emitVcpuAsm(opcode, "_" + numeric._name + " + 1" + operand, false); break;
    ...
    bool handleConditionOp(Expression::Numeric& lhs, Expression::Numeric& rhs, Expression::CondCode& condCode, bool& swappedLogic, bool& invertedLogic, const std::string& opcode="SUB")
    {
        // Swap left and right to take advantage of LDWI for 16bit numbers
        ...
        lhs._condCode._ccType = condCode._ccType;
        if((condCode._ccType == Expression::FastCC  &&  !condCode._positiveLogic)  ||  (condCode._ccType == Expression::NormalCC  &&  condCode._positiveLogic)) invertedLogic = true;
    ...
    bool handleStringAdd(Expression::Numeric& lhs, Expression::Numeric& rhs)
    {
        ...
        lhs =  Expression::Numeric(0, -1, true, false, false, Expression::TmpStrVar, {false, Expression::BooleanCC}, Expression::Int16Both, "", std::string(""));
    ...
    Expression::Numeric EQ(Expression::Numeric& left, Expression::Numeric& right, Expression::Numeric& input, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        left._isValid = handleConditionOp(left, right, input._condCode, swappedLogic, invertedLogic, "XOR");

        // Swap does nothing to EQ, invert changes EQ to NE
        std::string cc = (invertedLogic) ? "Ne" : "Eq";
        emitCcType(input._condCode._ccType, cc);
    ...
    Expression::Numeric LE(Expression::Numeric& left, Expression::Numeric& right, Expression::Numeric& input, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        bool swappedLogic = false;
        bool invertedLogic = false;
        left._isValid = handleConditionOp(left, right, input._condCode, swappedLogic, invertedLogic);
    ...
    Expression::Numeric POW(Expression::Numeric& left, Expression::Numeric& right, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        {
            return Expression::Numeric(0, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
        }
        // Optimise base = 1
        else if(left._varType == Expression::Number  &&  left._value == 1)
        {
            return Expression::Numeric(1, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
        }
        // Optimise exponent = 0
    ...
    Expression::Numeric MUL(Expression::Numeric& left, Expression::Numeric& right, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        // Optimise multiply with 1
        if(left._varType  == Expression::Number  &&  left._value  == 1) return right;
