...
namespace Memory
{
    ...
    int getSizeVRAM(void) {return _sizeFreeVRAM;}
    ...
    void initialise(void)
    {
        ...
        _sizeFreeVRAM = 0; for(int i=0; i<int(_videoRam.size()); i++) _sizeFreeVRAM += _videoRam[i]._size;
    ...
    void freeVideoRAM(void)
    {
        for(int i=0; i<int(_videoRam.size()); i++)
        {
            _freeRam.push_back(_videoRam[i]);
        }
    }
    ...
    bool updateFreeRamList(int index, uint16_t address, int size)
    {
        if(index >= 0  &&  size >= 0)
        {
            ...
            //Cpu::reportError(Cpu::NoError, stderr, "1 RAM segment: %3d  :  %04x %3d  :  %04x %3d\n", _sizeFreeRAM, address, size, uint16_t(address + size), newSize);
    ...
    bool updateFreeRamList(int index, uint16_t address0, int size0, uint16_t address1, int size1)
    {
        if(index >= 0)
        {
            ...
            //Cpu::reportError(Cpu::NoError, stderr, "2 RAM segments: %3d  :  %04x %3d  :  %04x %3d\n", _sizeFreeRAM, address0, size0, address1, size1);
    ...
    bool isFreeRAM(uint16_t address, int size)
    {
        ...
        {
            Cpu::reportError(Cpu::MemError, stderr, "Memory::isFreeRAM() : Memory at 0x%04x does not exist on this %d byte system : your request : 0x%04x %d\n", address, _sizeRAM, address, size);
            return false;
        }
    ...
    bool getNextCodeAddress(FitType fitType, uint16_t start, int size, uint16_t& address)
    {
        ...
        Cpu::reportError(Cpu::MemError, stderr, "Memory::getNextCodeAddress() : Couldn't find free code space in RAM of size %d bytes\n", size);
        return false;
    }
    ...
    bool takeFreeRAM(uint16_t address, int size, bool printError)
    {
        if(address > _sizeRAM - 1)
        {
            if(printError) Cpu::reportError(Cpu::MemError, stderr, "Memory::takeFreeRAM() : Memory at 0x%04x does not exist on this %d byte system : your request : 0x%04x %d\n", address, _sizeRAM, address, size);
            return false;
        }
        ...
        if(printError) Cpu::reportError(Cpu::MemError, stderr, "Memory::takeFreeRAM() : Memory at 0x%04x already in use : your request : 0x%04x %d\n", address, address, size);
        return false;
    }
    ...
    bool getFreeRAM(FitType fitType, int size, uint16_t min, uint16_t max, uint16_t& address, bool& withinPage)
    {
            ...
            case FitAscending:
            {
                for(int j=0; j<int(_freeRam.size()); j++)
                {
                    for(int i=0; i<_freeRam[j]._size; i++)
                    {
                        ...
                        {
                            address = addr;
                            withinPage = (HI_BYTE(addr) == HI_BYTE(addr + size-1));
        ...
        Cpu::reportError(Cpu::MemError, stderr, "Memory::getFreeRAM() : No free RAM found of size %d bytes\n", size);
        printFreeRamList(Memory::SizeDescending);
        return false;
    }

    // Attempts to returns RAM request of a given size : withinPage specifies no page boundary crossings : oddeven specifies type of address returned, 0=don't care, 1=even, 2=odd
    bool getFreePageRAM(FitType fitType, int size, uint16_t min, uint16_t max, uint16_t& address, bool withinPage, ParityType oddEven)
    {
        switch(fitType)
        {
            case FitAscending:
        ...
        Cpu::reportError(Cpu::MemError, stderr, "Memory::getFreePageRAM() : No free RAM found of size %d bytes\n", size);
        printFreeRamList(Memory::SizeDescending);
        return false;
    }

    // Return free RAM chunks in order of request : withinPage specifies no page boundary crossings
    bool getFreeChunkRAM(FitType fitType, uint16_t addrMin, uint16_t addrMax, uint16_t sizeMin, uint16_t& address, uint16_t inSize, uint16_t& outSize, bool withinPage)
    {
        switch(fitType)
        {
            case FitAscending:
        ...
        Cpu::reportError(Cpu::MemError, stderr, "Memory::getFreeChunkRAM() : No free RAM found within 0x%04x and 0x%04x\n", addrMin, addrMax);
        return false;
    }
    ...
    void printFreeRamList(SortType sortType)
    {
        ...
        for(int i=0; i<int(freeRam.size()); i++)
        {
            ...
            Cpu::reportError(Cpu::NoError, stderr, "Memory::printFreeRamList() : %3d : 0x%04x %3d\n", i, freeRam[i]._address, freeRam[i]._size);
        }
        Cpu::reportError(Cpu::NoError, stderr, "Memory::printFreeRamList() : Expected %5d : Found %5d\n", _sizeFreeRAM, totalFree);
    }
}
