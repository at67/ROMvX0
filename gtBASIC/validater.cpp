...
namespace Validater
{
                                   ...
                                   const std::string& opcode, const std::string& operand, const std::string& code, uint16_t address, int vasmSize)
    {
        if(itVasm >= itCode->_vasm.end())
        {
            ...
            Cpu::shutdown();
#endif
            Cpu::reportError(Cpu::ValError, stderr, "Validater::insertPageJumpInstruction() : Trying to insert a PAGE JUMP into lala land, in '%s'", itCode->_code.c_str());
            _EXIT_(EXIT_FAILURE);
        }

        Memory::takeFreeRAM(address, vasmSize);

        return itCode->_vasm.insert(itVasm, {address, opcode, operand, code, true, false, -1, vasmSize});
    }
...
#define CALLI_PAGE_JUMP_OFFSET 0
        ...
        if(opcode.size())
        {
            ...
            if(opcodeSize)
            {
                ...
                if(HI_BYTE(vPC) == HI_BYTE(vPC + opSize - 1)  &&  Memory::isFreeRAM(vPC, opSize))
                {
                    // Code relocation is not required if requested RAM address is free
                    Memory::takeFreeRAM(vPC, opcodeSize);
                    return false;
                }
                ...
                {
                    Cpu::reportError(Cpu::ValError, stderr, "Validater::checkForRelocation(): Memory alloc at 0x%0x4 of size %d failed\n", vPC, opSize);
                    return false;
                }
                ...
                if(!print)
                {
                    ...
                    Cpu::reportError(Cpu::NoError, stderr, "*                      Relocating                      \n");
                    Cpu::reportError(Cpu::NoError, stderr, "*******************************************************\n");
                    Cpu::reportError(Cpu::NoError, stderr, "*       Opcode         : Address :    Size     :  New  \n");
                    Cpu::reportError(Cpu::NoError, stderr, "*******************************************************\n");
                }
                ...
                Cpu::reportError(Cpu::NoError, stderr, "* %-20s : 0x%04x  :    %2d bytes : 0x%04x\n", opcode.c_str(), vPC, opcodeSize, newPC);
                return true;
            }
        }
    ...
    bool checkForRelocations(void)
    {
        ...
        for(auto itCode=Compiler::getCodeLines().begin(); itCode!=Compiler::getCodeLines().end();)
        {
            ...
            // Instruction was marked for a lazy erase, relocate everything after it
            for(auto itVasm=itCode->_vasm.begin(); itVasm!=itCode->_vasm.end();)
            {
                if(!itVasm->_pageJump  &&  itVasm->_lazyErase)
                {
                    Compiler::adjustLabelAddresses(itVasm->_address, -itVasm->_vasmSize);
                    Compiler::adjustVasmAddresses(codeLineIndex, itVasm->_address, -itVasm->_vasmSize);
                    itVasm = itCode->_vasm.erase(itVasm);
                }
                else
                {
                    itVasm++;
...
#define VAC_SAVE_START  0x00D6
                    ...
                    Compiler::adjustLabelAddresses(currPC, offset);
                    Compiler::adjustVasmAddresses(codeLineIndex, currPC, offset);

                    // Check for existing label, (after label adjustments)
                    int labelIndex = -1;
                    std::string labelName;
                    Compiler::VasmLine* vasmCurr = &itCode->_vasm[itVasm - itCode->_vasm.begin()]; // points to CALLI and LDW/LDLW
                    Compiler::VasmLine* vasmNext = &itCode->_vasm[itVasm + 1 - itCode->_vasm.begin()]; // points to instruction after CALLI and after LDW/LDLW
                    if(Compiler::findLabelAddr(nextPC) >= 0)
                    {
                        labelIndex = Compiler::findLabelAddr(nextPC);
                        labelName = Compiler::getLabels()[labelIndex]._name;
                    }
                    if(labelIndex == -1)
                    {
                        Compiler::Label label;
                        labelIndex = Compiler::createLabel(nextPC, nextPClabel, codeLineIndex, label, false, false, false);
    
                        // Create CALLI page jump label
                        if(Compiler::getCodeRomType() >= Cpu::ROMv5a)
                        {
                            //vasmCurr->_labelIndex = -1;
                            vasmNext->_labelIndex = labelIndex;
                        }
                        // Create pre-CALLI page jump label
                        else
                        {
                            vasmCurr->_labelIndex = labelIndex;
                            //vasmNext->_labelIndex = -1;
                        }
                    }
                    ...
                    else
                    {
                        ...
                        if(Compiler::getCodeRomType() >= Cpu::ROMv5a)
                        {
                            ...
                            vasmCurr->_code = (labelName[0] == '_') ? Expression::createPaddedString("CALLI",  OPCODE_TRUNC_SIZE, ' ') + labelName : Expression::createPaddedString("CALLI",  OPCODE_TRUNC_SIZE, ' ') + "_" + labelName;
                        }
                        ...
                        else
                        {
                            ...
                            vasm->_code = (labelName[0] == '_') ? Expression::createPaddedString("LDWI",  OPCODE_TRUNC_SIZE, ' ') + labelName : Expression::createPaddedString("LDWI",  OPCODE_TRUNC_SIZE, ' ') + "_" + labelName;
                        }
                    }
                }
        ...
        {
            Cpu::reportError(Cpu::NoError, stderr, "*******************************************************\n");
            Cpu::reportError(Cpu::NoError, stderr, "* Number of page jumps = %4d, RAM used = %5d bytes *\n", numThunks, totalThunkSize);
            Cpu::reportError(Cpu::NoError, stderr, "*******************************************************\n");
        }
    ...
    bool opcodeHasBranch(const std::string& opcode)
    {
        ...
        if(opcode == "DBNE")               return true;
        if(opcode == "DBGE")               return true;
        if(opcode == "%ForNextDBNE")       return true;
        if(opcode == "%ForNextDBGE")       return true;

        // ROMvX0 implements the following using the new JCC instructions
        if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD) return false;

        if(opcode == "%ForNextInc")        return true;
        if(opcode == "%ForNextDec")        return true;
        if(opcode == "%ForNextDecOne")     return true;
        ...
        for(int i=0; i<int(Compiler::getCodeLines().size()); i++)
        {
            ...
            for(int j=0; j<int(Compiler::getCodeLines()[i]._vasm.size()); j++)
            {
                ...
                if(opcodeHasBranch(opcode))
                {
                    ...
                    // Branch embedded in a DBNE/DBGE
                    else if(opcode == "DBNE"  ||  opcode == "DBGE")
                    {
    ...
    bool checkStatementBlocks(void)
    {
        ...
        while(!Compiler::getForNextDataStack().empty())
        {
            ...
            Cpu::reportError(Cpu::ValError, stderr, "Validater::checkStatementBlocks() : Syntax error, missing NEXT statement, for '%s' on line %d\n", code.c_str(), codeLineIndex);
        ...
        while(!Compiler::getElseIfDataStack().empty())
        {
            ...
            int codeLineIndex = elseIfData._codeIndex;
            const std::string& code = Compiler::getCodeLines()[codeLineIndex]._code;
            Cpu::reportError(Cpu::ValError, stderr, "Validater::checkStatementBlocks() : Syntax error, missing ELSE or ELSEIF statement, for '%s' on line %d\n", code.c_str(), codeLineIndex);
        ...
        while(!Compiler::getWhileWendDataStack().empty())
        {
            ...
            const std::string& code = Compiler::getCodeLines()[codeLineIndex]._code;
            Cpu::reportError(Cpu::ValError, stderr, "Validater::checkStatementBlocks() : Syntax error, missing WEND statement, for '%s' on line %d\n", code.c_str(), codeLineIndex);
            Compiler::getWhileWendDataStack().pop();
        }

        // Check REPEAT UNTIL blocks
        while(!Compiler::getRepeatUntilDataStack().empty())
        {
            ...
            const std::string& code = Compiler::getCodeLines()[codeLineIndex]._code;
            Cpu::reportError(Cpu::ValError, stderr, "Validater::checkStatementBlocks() : Syntax error, missing UNTIL statement, for '%s' on line %d\n", code.c_str(), codeLineIndex);
    ...
    bool checkCallProcFuncData(void)
    {
        ...
        for(auto it=Compiler::getCallDataMap().begin(); it!=Compiler::getCallDataMap().end(); ++it)
        {
            ...
            {
                Cpu::reportError(Cpu::ValError, stderr, "Validator::checkCallProcFuncData() : Syntax error, 'CALL <NAME>' cannot find a corresponding 'PROC <NAME>', in '%s' on line %d\n", code.c_str(), codeLineIndex);
                Cpu::reportError(Cpu::ValError, stderr, "Validator::checkCallProcFuncData() : Did you forget the comma between PROC name and it's parameters?\n");
                success = false;
                continue;
            }
            ...
            {
                Cpu::reportError(Cpu::ValError, stderr, "Validator::checkCallProcFuncData() : Syntax error, 'CALL <NAME>' has incorrect number of parameters compared to 'PROC <NAME>', in '%s' on line %d\n",
                                                        code.c_str(), codeLineIndex);
                success = false;
                continue;
            }
    ...
    bool checkRuntimeVersion(void)
    {
        ...
        {
            Cpu::reportError(Cpu::NoError, stderr, "\n*************************************************************************************************\n");
            Cpu::reportError(Cpu::NoError, stderr, "* Expected runtime version %04d : Found runtime version %04d\n", RUNTIME_VERSION, runtimeVersion);
            Cpu::reportError(Cpu::NoError, stderr, "*************************************************************************************************\n\n");
