...
namespace Functions
{
    ...
    Expression::Numeric IARR(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        else
        {
            ...
            if(numDims > MAX_ARRAY_DIMS)
            {
                Cpu::reportError(Cpu::FncError, stderr, "Functions::IARR() : '%s:%d' : %s() expects a maximum of %d dimension/s, found %d : %s\n", moduleName.c_str(), codeLineStart, numeric._name.c_str(), MAX_ARRAY_DIMS,
        ...
        if(numeric._varType == Expression::Number)
        {
            ...
            if(Expression::getOutputNumeric()._staticInit)
            {
                if(numeric._value == 0)
                {
                    ...
                    return Expression::Numeric(distribution(_randGenerator), -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
                }
    ...
    Expression::Numeric LEN(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(Expression::getOutputNumeric()._staticInit)
        {
            return Expression::Numeric(length, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
        }
        ...
        else
        {
            ...
            Expression::Numeric numericTmp = Expression::Numeric(int16_t(length), -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
        ...
        if(numeric._varType == Expression::String)
        {
            ...
            else if(sysVarName == "SPRITE_LUTS"  &&  numeric._params.size() == 1)
            {
                ...
                if(Expression::getOutputNumeric()._staticInit)
                {
                    switch(uint8_t(std::lround(numeric._params[0]._value)))
                    {
                        case 0:
                        {
                            numeric = Expression::Numeric(0x0000, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
            ...
            else if(numeric._params.size() == 0)
            {
                ...
                else if(sysVarName == "TIMER")
                {
                    if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD)
                    {
                        Compiler::emitVcpuAsm("LDW", "giga_secondsTickLo", false);
                    }
                    else
                    {
                ...
                else if(sysVarName == "JIFFIES")
                {
                    if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD)
                    {
                        Compiler::emitVcpuAsm("LD", "giga_jiffiesTick", false);
                    }
                    else
                    {
                        Compiler::emitVcpuAsm("LD", "timerJiff", false);
                    }
                }
    ...
    Expression::Numeric ABS(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(numeric._varType != Expression::String  &&  numeric._varType != Expression::StrVar &&  numeric._varType != Expression::TmpStrVar)
        {
            ...
            else
            {
                ...
                if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD)
                {
                    Compiler::emitVcpuAsm("ABSW", "", false);
                }
                else
                {
    ...
    Expression::Numeric SGN(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(numeric._varType != Expression::String  &&  numeric._varType != Expression::StrVar &&  numeric._varType != Expression::TmpStrVar)
        {
            ...
            else
            {
                ...
                if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD)
                {
                    Compiler::emitVcpuAsm("SGNW", "", false);
                }
                else
                {
    ...
    Expression::Numeric ASC(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(Expression::getOutputNumeric()._staticInit)
        {
            return Expression::Numeric(ascii, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
        }
    ...
    Expression::Numeric TWIDTH(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        if(Expression::getOutputNumeric()._staticInit)
        {
            Cpu::reportError(Cpu::FncError, stderr, "Functions::TWIDTH() : '%s:%d' : TWIDTH() cannot be used in static initialisation : %s\n", moduleName.c_str(), codeLineStart, codeLineText.c_str());
            numeric._isValid = false;
            return numeric;
        }
        ...
        if(numeric._varType == Expression::Number)
        {
            Cpu::reportError(Cpu::FncError, stderr, "Functions::TWIDTH() : '%s:%d' : parameter can't be a literal : %s\n", moduleName.c_str(), codeLineStart, codeLineText.c_str());
            numeric._isValid = false;
            return numeric;
        }
        ...
        if(numeric._index == -1)
        {
            switch(numeric._varType)
            {
                ...
                default:
                {
                    Cpu::reportError(Cpu::FncError, stderr, "Functions::TWIDTH() : '%s:%d' : couldn't find variable name '%s' : %s\n", moduleName.c_str(), codeLineStart, numeric._name.c_str(), codeLineText.c_str());
                    numeric._isValid = false;
                    return numeric;
                }
            }
        ...
        switch(numeric._varType)
        {
            case Expression::StrVar:   size = Compiler::getStringVars()[numeric._index]._size; break;
            case Expression::Constant: size = Compiler::getConstants()[numeric._index]._size;  break;
        ...
        {
            // Generate code to save result into a tmp var
            Compiler::emitVcpuAsm("LDI", std::to_string(size), false);
        }

        Compiler::emitVcpuAsm("%TextWidth", "", false);
    ...
    Expression::Numeric STRCMP(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(numeric._varType == Expression::String  &&  numeric._params[0]._varType == Expression::String)
        {
            ...
            if(Expression::getOutputNumeric()._staticInit)
            {
                ...
                return Expression::Numeric(value, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
            }
            // Generate code to save result into a tmp var
            else
            {
                int result = int(numeric._text == numeric._params[0]._text) - 1;
                Expression::Numeric numericTmp = Expression::Numeric(int16_t(result), -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
                Compiler::emitVcpuAsmForLiteral(numericTmp, false);
            }
        }
        else
    ...
    Expression::Numeric VAL(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(numeric._varType == Expression::String)
        {
            ...
            if(Expression::getOutputNumeric()._staticInit)
            {
                return Expression::Numeric(val, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
            }
            // Generate code to save result into a tmp var
            else
            {
                Expression::Numeric numericTmp = Expression::Numeric(val, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
                Compiler::emitVcpuAsmForLiteral(numericTmp, false);
            }
        }
        else
        {
            ...
            // IntegerStr expects srcAddr to point past the string's length byte
    ...
    Expression::Numeric ADDR(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(Expression::getOutputNumeric()._staticInit)
        {
            ...
            return Expression::Numeric(address, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
        }
    ...
    Expression::Numeric POINT(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD)
        {
            ...
            Compiler::getNextTempVar();
            Operators::changeToTmpVar(numeric);
            Compiler::emitVcpuAsm("LDPX", "", false);
            Compiler::emitVcpuAsm("STW", Expression::byteToHexString(uint8_t(Compiler::getTempVarStart())), false);
        }
        else
        {
    ...
    Expression::Numeric COND(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD)
        {
            if((numeric._params[0]._varType == Expression::Number  &&  numeric._params[1]._varType == Expression::Number)  &&
               (numeric._params[0]._value >= 0  &&  numeric._params[0]._value <= 255  &&  numeric._params[1]._value >= 0  &&  numeric._params[1]._value <= 255))
            {
                uint8_t imm0 = int8_t(std::lround(numeric._params[0]._value));
                uint8_t imm1 = int8_t(std::lround(numeric._params[1]._value));
                Operators::createSingleOp("LDW", numeric);
                Compiler::emitVcpuAsm("CONDII", std::to_string(imm0) + ", " + std::to_string(imm1), false);
            ...
            else if(numeric._params[0]._varType == Expression::Number  &&  numeric._params[0]._value >= 0  &&  numeric._params[0]._value <= 255)
            {
                uint8_t imm = int8_t(std::lround(numeric._params[0]._value));
                std::string var = Operators::getVarAsOperand(numeric._params[1]);
                Operators::createSingleOp("LDW", numeric);
                Compiler::emitVcpuAsm("CONDIB", std::to_string(imm) + ", " + var, false);
            ...
            else if(numeric._params[1]._varType == Expression::Number  &&  numeric._params[1]._value >= 0  &&  numeric._params[1]._value <= 255)
            {
                std::string var = Operators::getVarAsOperand(numeric._params[0]);
                uint8_t imm = int8_t(std::lround(numeric._params[1]._value));
                Operators::createSingleOp("LDW", numeric);
                Compiler::emitVcpuAsm("CONDIB", var + ", " +  std::to_string(imm), false);
            ...
            else
            {
                ...
                Operators::createSingleOp("LDW", numeric);
                Compiler::emitVcpuAsm("CONDBB", var0 + ", " +  var1, false);
        ...
        std::string condText = "_cond_" + Expression::wordToHexString(Compiler::getVasmPC());
        ...
        if(numeric._params[0]._varType == Expression::Number  &&  numeric._params[1]._varType == Expression::Number)
        {
            ...
            Compiler::emitVcpuAsm("%IntCondii", condText + " " + imm0 + " " + imm1, false);
        }
        ...
        else if(numeric._params[0]._varType != Expression::Number  &&  numeric._params[1]._varType == Expression::Number)
        {
            ...
            Compiler::emitVcpuAsm("%IntCondvi", condText + " " + var + " " + imm, false);
        }
        ...
        else if(numeric._params[0]._varType == Expression::Number  &&  numeric._params[1]._varType != Expression::Number)
        {
            ...
            std::string var = Operators::getVarAsOperand(numeric._params[1]);
            Compiler::emitVcpuAsm("%IntCondiv", condText + " " + imm + " " + var, false);
        }
        else
        {
            ...
            std::string var0 = Operators::getVarAsOperand(numeric._params[0]);
            std::string var1 = Operators::getVarAsOperand(numeric._params[1]);
            Compiler::emitVcpuAsm("%IntCondvv", condText + " " + var0 + " " + var1, false);
        }
    ...
    Expression::Numeric SQRT(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(numeric._varType != Expression::String  &&  numeric._varType != Expression::StrVar &&  numeric._varType != Expression::TmpStrVar)
        {
            ...
            else
            {
                if(Compiler::getCodeRomType() < Cpu::ROMvX0  ||  Compiler::getCodeRomType() >= Cpu::SDCARD)
                {
                    Cpu::reportError(Cpu::FncError, stderr, "Functions::SQRT() : '%s:%d' : version error, SQRT() requires ROMvX0 : %s\n", moduleName.c_str(), codeLineStart, codeLineText.c_str());
                    numeric._isValid = false;
                    return numeric;
                }
    ...
    Expression::Numeric CHR$(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(numeric._varType == Expression::Number)
        {
            ...
            return Expression::Numeric(0, uint16_t(-1), true, false, false, Expression::TmpStrVar, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
        }
    ...
    Expression::Numeric SPC$(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(numeric._varType == Expression::Number)
        {
            ...
            if(len < 1  ||  len > USER_STR_SIZE)
            {
                Cpu::reportError(Cpu::FncError, stderr, "Functions::SPC$() : '%s:%d' : syntax error, 'SPC$(n)', if 'n' is a literal, it MUST be <1-%d> : %s\n", moduleName.c_str(), codeLineStart, USER_STR_SIZE, codeLineText.c_str());
                numeric._isValid = false;
                return numeric;
            }
    ...
    Expression::Numeric STRING$(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(numeric._varType == Expression::Number)
        {
            ...
            return Expression::Numeric(numeric._value, uint16_t(-1), true, false, false, Expression::StrAddr, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
        }
    ...
    Expression::Numeric TIME$(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        // Enable time handler
        Compiler::emitVcpuPreProcessingCmd("%define TIME_HANDLER");
    ...
    Expression::Numeric LEFT$(Expression::Numeric& numeric, const std::string& moduleName, const std::string& codeLineText, int codeLineStart)
    {
        ...
        if(numeric._varType == Expression::String  &&  numeric._params[0]._varType == Expression::Number)
        {
            ...
            return Expression::Numeric(0, uint16_t(index), true, false, false, Expression::StrVar, {false, Expression::BooleanCC}, Expression::Int16Both, name, std::string(""));
        }
        ...
        return Expression::Numeric(0, uint16_t(-1), true, false, false, Expression::TmpStrVar, {false, Expression::BooleanCC}, Expression::Int16Both, name, std::string(""));
    }
