...
namespace Expression
{
    ...
    Numeric& operatorLT(Numeric& left, Numeric& right, Numeric& input)
    {
    ...
    Numeric& operatorGT(Numeric& left, Numeric& right, Numeric& input)
    {
        UNREFERENCED_PARAM(input);
    ...
    Numeric& operatorEQ(Numeric& left, Numeric& right, Numeric& input)
    {
        UNREFERENCED_PARAM(input);
    ...
    Numeric& operatorNE(Numeric& left, Numeric& right, Numeric& input)
    {
        UNREFERENCED_PARAM(input);
    ...
    Numeric& operatorLE(Numeric& left, Numeric& right, Numeric& input)
    {
        UNREFERENCED_PARAM(input);
    ...
    Numeric& operatorGE(Numeric& left, Numeric& right, Numeric& input)
    {
        UNREFERENCED_PARAM(input);
    ...
    {
        bool success = true;
        std::string inp = strUpper(input);
        for(int i=0; i<int(inp.size()); i++) success &= _hexaDecimalChars[uint8_t(inp.c_str()[i])];
        return success;
    }
    ...
    void removeAsmComments(std::string& inout)
    {
        size_t pos = inout.find(';');
    ...
    void isInComment(const std::string& input, int index, bool& output)
    {
        ...
        // In comment if trailing a single quote or if NOT a start char literal or if trailing a 'REM' sequence
           ...
           (index <= int(input.size()) - 3  &&  toupper((unsigned char)input[index]) == 'R'  &&  toupper((unsigned char)input[index+1]) == 'E'  &&  toupper((unsigned char)input[index+2]) == 'M'))
        {
            ...
            // Not in comment if an end char literal
    ...
    std::string& stripWhitespaceInPlace(std::string& input)
    {
        input.erase(remove_if(input.begin(), input.end(), isSpace), input.end());
    ...
    std::string& stripNonStringWhitespaceInPlace(std::string& input)
    {
        initHasHelpers();
        input.erase(remove_if(input.begin(), input.end(), hasNonStringWhiteSpace), input.end());
    ...
    bool stripChars(std::string& input, const std::string& chars)
    {
        ...
        {
            output.erase(std::remove(output.begin(), output.end(), chars[i]), output.end());
        }
    ...
    std::string createPaddedString(const std::string& input, int size, char pad)
    {
        if(size < input.size())
        {
            Cpu::reportError(Cpu::XprError, stderr, "Expression::padString() : size %d < %s.size() %d\n", size, input.c_str(), input.size());
            return "";
        }

        return input + std::string(size - input.size(), pad);
    }
    ...
    NumericType getBase(const std::string& input, long& result)
    {
        ...
        // Multiple operands are separated by comma's in instructions such as XCHGB, DBNE, etc
        stripChars(token, ",");
        ...
        if(token.size() >= 2  &&  token.c_str()[0] == '$')
        {
            ...
            if(success)
            {
                result = strtoul(&token.c_str()[1], NULL, 16);
                return HexaDecimal; 
            }
        }
        // Hex
        else if(token.size() >= 3  &&  ((token.c_str()[0] == '0'  &&  token.c_str()[1] == 'X')  ||  (token.c_str()[0] == '&'  &&  token.c_str()[1] == 'H')))
        {
            ...
            if(success)
            {
                result = strtoul(&token.c_str()[2], NULL, 16);
                return HexaDecimal; 
            }
        }
        // Octal
        else if(token.size() >= 3  &&  ((token.c_str()[0] == '0'  &&  (token.c_str()[1] == 'O' || token.c_str()[1] == 'Q'))  ||  (token.c_str()[0] == '&'  &&  token.c_str()[1] == 'O')))
        {
            ...
            if(success)
            {
                result = strtoul(&token.c_str()[2], NULL, 8);
                return Octal; 
            }
        }
        // Binary
        else if(token.size() >= 3  &&  ((token.c_str()[0] == '0'  &&  token.c_str()[1] == 'B')  ||  (token.c_str()[0] == '&'  &&  token.c_str()[1] == 'B')))
        {
            ...
            if(success)
            {
                result = strtoul(&token.c_str()[2], NULL, 2);
                return Binary; 
            }
        }
        // Decimal
        else
        {
            ...
            if(success)
            {
                result = strtoul(&token.c_str()[0], NULL, 10);
                return Decimal; 
            }
        }
    ...
    bool stringToI32(const std::string& token, int32_t& result)
    {
        if(token.size() < 1  ||  token.size() > 34) return false;

        long lResult;
        NumericType base = getBase(token, lResult);
        if(base == BadBase) return false;

        result = int32_t(lResult);
        return true;
    }
    ...
    bool stringToU32(const std::string& token, uint32_t& result)
    {
        if(token.size() < 1  ||  token.size() > 34) return false;
        ...
        for(int i=0; i<=int(line.size()); i++)
        {
            ...
            else
            {
                ...
                if(strchr(delimiterStr.c_str(), line[i]))
                {
                    if(delimiterState != Quotes)
                    {
                        // Save comma's for further processing
    ...
    int replaceText(std::string& inout, const std::string& text, const std::string& replace, size_t offset)
    {
        int replaced = 0;
        for(size_t foundPos=offset; ; foundPos+=replace.size())
        {
            foundPos = inout.find(text, foundPos);
            if(foundPos == std::string::npos) break;

            inout.erase(foundPos, text.size());
            inout.insert(foundPos, replace);
            ++replaced;
        }
    ...
    char get(void)
    {
        ...
        //Cpu::reportError(Cpu::NoError, stderr, "%s : %s : %c\n", _expressionToParse.c_str(), _expression, chr);
        advance(1);
        return chr;
    }
    ...
    Numeric factor(Numeric& input, int16_t defaultValue)
    {
        int16_t value = 0;
        Numeric numeric;
        ...
        if(peek() == '(')
        {
            ...
            if(peek() != ')')
            {
                Cpu::reportError(Cpu::XprError, stderr, "Expression::factor() : %d : '%s' : missing ')'\n", _lineNumber + 1, _expressionToParse.c_str());
                numeric = Numeric();
            }
            get();
        }
        else if((peek() >= '0'  &&  peek() <= '9')  ||  peek() == '$'  ||  peek() == '&')
        {
            if(!number(value))
            {
                Cpu::reportError(Cpu::XprError, stderr, "Expression::factor() : %d : '%s' : bad numeric data\n", _lineNumber + 1, _expressionToParse.c_str());
                numeric = Numeric();
            }
            else
            {
                numeric = Numeric(value, -1, true, false, false, Number, {false, BooleanCC}, Int16Both, std::string(""), std::string(""));
            }
        }
        // Functions
        else if(find("POW"))
        {
            numeric = factor(input, defaultValue); numeric = operatorPOWF(numeric);
        }
        else if(find("SQRT"))
        {
            numeric = factor(input, defaultValue); numeric = operatorSQRT(numeric);
        }
        else if(find("EXP2"))
        {
            numeric = factor(input, defaultValue); numeric = operatorEXP2(numeric);
        }
        else if(find("EXP"))
        {
            numeric = factor(input, defaultValue); numeric = operatorEXP(numeric);
        }
        else if(find("LOG10"))
        {
            numeric = factor(input, defaultValue); numeric = operatorLOG10(numeric);
        }
        else if(find("LOG2"))
        {
            numeric = factor(input, defaultValue); numeric = operatorLOG2(numeric);
        }
        else if(find("LOG"))
        {
            numeric = factor(input, defaultValue); numeric = operatorLOG(numeric);
        }
        else if(find("SIN"))
        {
            numeric = factor(input, defaultValue); numeric = operatorSIN(numeric);
        }
        else if(find("COS"))
        {
            numeric = factor(input, defaultValue); numeric = operatorCOS(numeric);
        }
        else if(find("TAN"))
        {
            numeric = factor(input, defaultValue); numeric = operatorTAN(numeric);
        }
        else if(find("ASIN"))
        {
            numeric = factor(input, defaultValue); numeric = operatorASIN(numeric);
        }
        else if(find("ACOS"))
        {
            numeric = factor(input, defaultValue); numeric = operatorACOS(numeric);
        }
        else if(find("ATAN2"))
        {
            numeric = factor(input, defaultValue); numeric = operatorATAN2(numeric);
        }
        else if(find("ATAN"))
        {
            numeric = factor(input, defaultValue); numeric = operatorATAN(numeric);
        }
        //else if(find("RAND"))
        //{
        //    numeric = factor(input, defaultValue); numeric = operatorRAND(numeric);
        //}
        else if(find("REV16"))
        {
            numeric = factor(input, defaultValue); numeric = operatorREV16(numeric);
        }
        else if(find("REV8"))
        {
            numeric = factor(input, defaultValue); numeric = operatorREV8(numeric);
        }
        else if(find("REV4"))
        {
            numeric = factor(input, defaultValue); numeric = operatorREV4(numeric);
        }
        else
        {
            // Unary operators
            switch(peek())
            {
                case '+': get(); numeric = factor(input, defaultValue);                                 break;
                case '-': get(); numeric = factor(input, defaultValue); numeric = operatorNEG(numeric); break;
                case '~': get(); numeric = factor(input, defaultValue); numeric = operatorNOT(numeric); break;

                // Unknown
                default: numeric = Numeric(defaultValue, -1, true, false, false, Number, {false, BooleanCC}, Int16Both, std::string(_expression), std::string("")); break;
            }
        }
    ...
    Numeric term(Numeric& input)
    {
        Numeric init, result = factor(input, 0);

        for(;;)
        {
            if(find("**"))         {       init = factor(input, 0); result = operatorPOW(result, init);}
            else if(peek() == '*') {get(); init = factor(input, 0); result = operatorMUL(result, init);}
            else if(peek() == '/') {get(); init = factor(input, 0); result = operatorDIV(result, init);}
            else if(peek() == '%') {get(); init = factor(input, 0); result = operatorMOD(result, init);}
            else return result;
        }
    }

    Numeric expr(Numeric& input)
    {
        Numeric init, result = term(input);
    
        for(;;)
        {
            if(peek() == '+')      {get(); init = term(input); result = operatorADD(result, init);}
            else if(peek() == '-') {get(); init = term(input); result = operatorSUB(result, init);}
            else return result;
        }
    }

    Numeric logical(Numeric& input)
    {
        Numeric init, result = expr(input);
    
        for(;;)
        {
            if(peek() == '&')      {get(); init = expr(input); result = operatorAND(result, init);}
            else if(peek() == '^') {get(); init = expr(input); result = operatorXOR(result, init);}
            else if(peek() == '|') {get(); init = expr(input); result = operatorOR(result,  init);}
            else if(find("<<"))    {       init = expr(input); result = operatorLSL(result, init);}
            else if(find(">>"))    {       init = expr(input); result = operatorLSR(result, init);}
            else return result;
        }
    }

    Numeric expression(Numeric& input)
    {
        Numeric init, result = logical(input);
            
        for(;;)
        {
            if(find("=="))         {       init = logical(input); result = operatorEQ(result, init, input);}
            else if(find("!="))    {       init = logical(input); result = operatorNE(result, init, input);}
            else if(find("<="))    {       init = logical(input); result = operatorLE(result, init, input);}
            else if(find(">="))    {       init = logical(input); result = operatorGE(result, init, input);}
            else if(peek() == '<') {get(); init = logical(input); result = operatorLT(result, init, input);}
            else if(peek() == '>') {get(); init = logical(input); result = operatorGT(result, init, input);}
            else return result;
        }
    }
