...
namespace Compiler
{
    ...
    InternalLabel _nextInternalLabel = {0x0000, ""};
    ...
    const InternalLabel& getNextInternalLabel(void) {return _nextInternalLabel;}
    ...
    {
        if(_nextInternalLabel._name.size())
        {
            _nextInternalLabel._address = address;
        }
    }
    ...
    void setNextInternalLabel(const std::string& labelName)
    {
        if(_nextInternalLabel._name.size())
        {
            _discardedLabels.push_back(_nextInternalLabel);
        }

        _nextInternalLabel._address = _vasmPC;
        _nextInternalLabel._name = labelName;
    }

    void adjustDiscardedLabels(const std::string& labelName, uint16_t address)
    {
        // TODO: fix this hack, should be using InternalLabels._address field
        uint16_t match;
        Expression::stringToU16(labelName.substr(labelName.size() - 6, 6), match);
    ...
    void adjustLabelAddresses(uint16_t address, int offset)
    {
        ...
        {
            if(_internalLabels[i]._address >= address)
            {
                _internalLabels[i]._address += int16_t(offset);
            }
        }
    }
    ...
    // Check if discarded label exists as an internal label with a different address
    bool checkDiscardedAddress(const std::string& name)
    {
        uint16_t discardAddr = 0, internalAddr = 0;

        for(int i=0; i<int(_discardedLabels.size()); i++)
        {
            if(name == _discardedLabels[i]._name)
            {
                discardAddr = _discardedLabels[i]._address;
                break;
            }
        }
        ...
        for(int i=0; i<int(_internalLabels.size()); i++)
        {
            if(name == _internalLabels[i]._name)
            {
                internalAddr = _internalLabels[i]._address;
                break;
            }
        }

        if(discardAddr  &&  internalAddr  &&  discardAddr != internalAddr) return true;
    ...
    void createLabel(uint16_t address, const std::string& name, int codeLineIndex, Label& label, bool numeric, bool addUnderscore, bool pageJump, bool gosub, bool noPush)
    {
        std::string n = name;
    ...
    bool createAsmLine(const std::string& code)
    {
        ...
        _codeLines[_currentCodeLineIndex]._vasm.push_back({uint16_t(_vasmPC - vasmSize), opcodeStr, operandStr, line, _nextInternalLabel._name, false, false, vasmSize});
        _codeLines[_currentCodeLineIndex]._vasmSize += vasmSize;
        _codeLines[_currentCodeLineIndex]._dontParse = true;
        _nextInternalLabel = {0x0000, ""};
    ...
    std::pair<int, int> emitVcpuAsm(const std::string& opcodeStr, const std::string& operandStr, bool nextTempVar, int codeLineIdx, const std::string& labelName, bool pageJump)
    {
        ...
        std::string intLabel = (_nextInternalLabel._name.size()) ? _nextInternalLabel._name : labelName;

        // Discarded labels are replaced correctly later in outputLabels()
        if(_nextInternalLabel._name.size()  &&  labelName.size()) _discardedLabels.push_back({_vasmPC, labelName});

        // Breakpoint instruction
        static int breakPointLine = -1;
        static int breakPointSlot = -1;
        if(intLabel.size()  &&  Expression::strUpper(opcodeStr) == "_BREAKPOINT_")
        {
            breakPointLine = codeLineIdx;
            breakPointSlot = int(_codeLines[codeLineIdx]._vasm.size());
            _codeLines[codeLineIdx]._vasm.push_back({uint16_t(_vasmPC - vasmSize), opcodeStr, operandStr, line, intLabel, pageJump, false, vasmSize});
        }
        else
        {
            ...
            {
                std::string iLabel = _codeLines[breakPointLine]._vasm[breakPointSlot]._internalLabel;
                _codeLines[breakPointLine]._vasm[breakPointSlot] = VasmLine({uint16_t(_vasmPC - vasmSize), opcodeStr, operandStr, line, iLabel, pageJump, false, vasmSize});
                _codeLines[codeLineIdx]._vasm.push_back({_vasmPC, "_breakpoint_", "", "_breakpoint_", "", false, 0});
                breakPointLine = -1;
                breakPointSlot = -1;
            }
            // Normal instruction
    ...
    void createVcpuAsmLabel(int codeLineIdxBra, int vcpuAsmBra, int codeLineIdxDst, int vcpuAsmDst, const std::string& label)
    {
        ...
        _codeLines[codeLineIdxBra]._vasm[vcpuAsmBra]._code = Expression::createPaddedString(opcode, OPCODE_TRUNC_SIZE, ' ') + label;
    ...
    LabelResult checkForLabel(const std::string& moduleName, std::string& code, int lineNumber)
    {
        ...
        if(code.size() > 1  &&  isdigit((unsigned char)code[0]))
        {
            ...
            createLabel(_vasmPC, labelName, int(_codeLines.size()), label, numeric, true, false, foundGosub, noPushGosub);
            if(!createCodeLine(code, int(space + 1), _currentLabelIndex, -1, Expression::Int16Both, false, codeLine, moduleName))
            {
        ...
        if(colon != std::string::npos)
        {
            ...
            if(Expression::isLabNameValid(labelName))
            {
                ...
                createLabel(_vasmPC, labelName, int(_codeLines.size()), label, false, true, false, foundGosub, noPushGosub);
                ...
                if(code.size() > colon + 1)
                {
                    ...
                    if(labelCode.size() > 2)
                    {
                        if(!createCodeLine(code, int(colon  + 1), _currentLabelIndex, -1, Expression::Int16Both, false, codeLine, moduleName))
                        {
                            Cpu::reportError(Cpu::ComError, stderr, "Compiler::checkForLabel() : '%s:%d' : createCodeLine() failed in '%s'\n", moduleName.c_str(), lineNumber + 1, code.c_str());
                            return LabelError;
                        }
    ...
    Expression::Numeric factor(int16_t defaultValue, bool returnAddress=false)
    {
        ...
        {
            numeric = factor(0); numeric = Functions::COND(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("FMUL"))
        {
            numeric = factor(0); numeric = Functions::FMUL(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("FDIV"))
        {
            numeric = factor(0); numeric = Functions::FDIV(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("FMOD"))
        {
            numeric = factor(0); numeric = Functions::FMOD(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("FDIVMOD"))
        {
            numeric = factor(0); numeric = Functions::FDIVMOD(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("SQRT"))
        {
            numeric = factor(0); numeric = Functions::SQRT(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        ...
        {
            numeric = factor(0); numeric = Functions::LCASE$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("UCASE$"))
        {
            numeric = factor(0); numeric = Functions::UCASE$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
...
REDO_STATEMENT:
    ...
    void addLabelToBooleanCC(Compiler::VasmLine& vasm, const std::string& opcode, const std::string& label)
    {
        ...
        if(opcode.size()  &&  opcode[0] == '%') opc = opcode.substr(1);

        vasm._code = Expression::createPaddedString(opc, OPCODE_TRUNC_SIZE, ' ') + label;
        vasm._opcode = opcode;
        vasm._operand = label;
    }
        ...
        for(int i=0; i<int(vasm.size()); i++)
        {
            if(_codeRomType >= Cpu::ROMv5a)
            {
                if(vasm[i]._code.find("CALLI_JUMP") != std::string::npos)
                {
                    ...
                    vasm[i]._operand = label;
                    vasm[i]._code = Expression::createPaddedString("CALLI", OPCODE_TRUNC_SIZE, ' ') + label;
                    return;
                }
            }
            else
            {
                if(vasm[i]._code.find("BRA_JUMP") != std::string::npos)
                {
                    vasm[i]._opcode = "BRA";
                    vasm[i]._operand = label;
                    vasm[i]._code = Expression::createPaddedString("BRA", OPCODE_TRUNC_SIZE, ' ') + label;
                    return;
                }
                else if(vasm[i]._code.find("LDWI_JUMP") != std::string::npos)
                {
                    vasm[i]._opcode = "LDWI";
                    vasm[i]._operand = label;
                    vasm[i]._code = Expression::createPaddedString("LDWI", OPCODE_TRUNC_SIZE, ' ') + label;
                    return;
                }
            }
        }
    ...
    void outputLabels(void)
    {
        ...
        for(int i=0; i<int(_codeLines.size()); i++)
        {
            for(int j=0; j<int(_codeLines[i]._vasm.size()); j++)
            {
                ...
                if(internalLabel.size())
                {
                    ...
                    _output.push_back(Expression::createPaddedString(internalLabel, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + addrStr + "\n");
        ...
        for(int i=0; i<int(_codeLines.size()); i++)
        {
            for(int j=0; j<int(_codeLines[i]._vasm.size()); j++)
            {
                // Check for BASIC label conflicts
                ...
                // Update discarded labels
                for(int k=0; k<int(_discardedLabels.size()); k++)
                {
                    int labelIdx = -1;
                    for(int l=0; l<int(_internalLabels.size()); l++)
                    {
                        // Match on unique address embedded within names or the real address
                        uint16_t internalAddr, discardedAddr;
                        bool nameAddrMatch = (getAddressFromName(_internalLabels[l]._name, internalAddr)  &&  getAddressFromName(_discardedLabels[k]._name, discardedAddr)  &&  (internalAddr == discardedAddr));

                        // Check discarded labels to be replaced
                        if(nameAddrMatch  ||  (_internalLabels[l]._address == _discardedLabels[k]._address))
                        {
                            // Check if discarded label exists as an internal label with a different address
                            if(checkDiscardedAddress(_discardedLabels[k]._name)) continue;

                            labelIdx = l;

                            // Non page labels get priority
                            if(_internalLabels[l]._name.substr(0, 6) != "_page_") break;
                        }
                    }

                    if(labelIdx < 0) continue;

                    // Replace discarded label with internal label                
                    Expression::replaceText(_codeLines[i]._vasm[j]._code, _discardedLabels[k]._name, _internalLabels[labelIdx]._name);
                    ...
                    {
                        std::string replacedText = _discardedLabels[k]._name.substr(1);      // ignore leading '_'
                        std::string replaceText = _internalLabels[labelIdx]._name.substr(1); // ignore leading '_'
    ...
    void outputInternalEquates(void)
    {
        ...
        _output.push_back(Expression::createPaddedString("timerTick",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x2A\n");
        _output.push_back(Expression::createPaddedString("timerPrev",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x2C\n");
        _output.push_back("\n");
        ...
        for(int i=0; i<int(_codeLines.size()); i++)
        {
            ...
            if(_codeLines[i]._code.size() >= 2  &&  _codeLines[i]._vasm.size())
            {
                ...
                for(int j=0; j<int(_codeLines[i]._vasm.size()); j++)
                {
                    ...
                    std::string label = (useBasicLabel) ? basicLabel : Expression::createPaddedString(vasmLabel, LABEL_TRUNC_SIZE, ' ');
