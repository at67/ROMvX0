...
RESTART_PRINT:
    ...
    bool INPUT(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        {
            Cpu::reportError(Cpu::KwdError, stderr, "Keywords::INPUT() : '%s:%d' : syntax error, use 'INPUT <heading>, <int/str var0>, <prompt0>, ... <int/str varN>, <promptN>' : %s\n",
                                                    codeLine._moduleName.c_str(), codeLineStart, codeLine._text.c_str());
            return false;
        }
    ...
    bool FOR(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        if(optimise)
        {
            ...
            // Variable start
            if(optimise  &&  varStart  &&  endNumeric._isValid  &&  constEnd  &&  loopEnd >= 0  &&  loopEnd <= 255)
            {
            ...
            else if(romvx  &&  optimise  &&  abs(loopStep) == 1)
            {
                ...
                {
                    // Loop start
                    Expression::Numeric numericTmp = Expression::Numeric(int16_t(loopStart), -1, true, false, false, Expression::Number, Expression::BooleanCC, Expression::Int16Both, std::string(""), std::string(""));
            ...
            else
            {
                ...
                // Loop end
                numericTmp = Expression::Numeric(int16_t(loopEnd), -1, true, false, false, Expression::Number, Expression::BooleanCC, Expression::Int16Both, std::string(""), std::string(""));
                Compiler::emitVcpuAsmForLiteral(numericTmp, Expression::byteToHexString(uint8_t(varEnd)), false);

                // Loop step
                numericTmp = Expression::Numeric(int16_t(loopStep), -1, true, false, false, Expression::Number, Expression::BooleanCC, Expression::Int16Both, std::string(""), std::string(""));
        ...
        Compiler::getForNextDataStack().push({varCounter, Compiler::getNextInternalLabel()._name, loopStart, loopEnd, loopStep, varEnd, varStep, type, farJump, optimise, romvx, codeLineIndex});
                    ...
                    case Compiler::DownTo:
                    {
                        ...
                        if(romvx)
                        {
                            ...
                            else if(loopEnd == 1)
                            {
                                ...
                                {
                                    Compiler::emitVcpuAsm("%ForNextDecOne", "_" + varName + " " + labName, false);
                                }
                            }
    ...
    bool IF(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        std::string nextInternalLabel = Compiler::getNextInternalLabel()._name;
        if(Compiler::getCodeRomType() < Cpu::ROMvX0  ||  Compiler::getCodeRomType() >= Cpu::SDCARD) nextInternalLabel += " " + std::to_string(Compiler::getNextJumpFalseUniqueId());

        // Update if's jump to this new label
        switch(condition._ccType)
        {
            case Expression::BooleanCC: addLabelToBooleanCC(Compiler::getCodeLines()[codeLineIndex]._vasm[jmpIndex], "%JumpFalse", nextInternalLabel); break;
            case Expression::NormalCC:  addLabelToJumpCC(Compiler::getCodeLines()[codeLineIndex]._vasm, nextInternalLabel);                            break;
            case Expression::FastCC:    addLabelToJumpCC(Compiler::getCodeLines()[codeLineIndex]._vasm, Compiler::getNextInternalLabel()._name);       break;
    ...
    bool ELSEIF(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        switch(ccType)
        {
            case Expression::BooleanCC: addLabelToBooleanCC(Compiler::getCodeLines()[codeIndex]._vasm[jmpIndex], "%JumpFalse", nextInternalLabel); break;
            case Expression::NormalCC:  addLabelToJumpCC(Compiler::getCodeLines()[codeIndex]._vasm, nextInternalLabel);                            break;
            case Expression::FastCC:    addLabelToJumpCC(Compiler::getCodeLines()[codeIndex]._vasm, Compiler::getNextInternalLabel()._name);       break;
        ...
        while(!endIfData.empty())
        {
            ...
            switch(ccType)
            {
                case Expression::BooleanCC: 
                {
                    if(Compiler::getCodeRomType() >= Cpu::ROMv5a)
                    {
                        addLabelToBooleanCC(Compiler::getCodeLines()[codeIndex]._vasm[jmpIndex], "CALLI", Compiler::getNextInternalLabel()._name);
                    }
                    else
                    {
                        addLabelToBooleanCC(Compiler::getCodeLines()[codeIndex]._vasm[jmpIndex], "LDWI", Compiler::getNextInternalLabel()._name);
                    }
                }
                break;

                case Expression::NormalCC:  addLabelToJump(Compiler::getCodeLines()[codeIndex]._vasm, Compiler::getNextInternalLabel()._name); break;
                case Expression::FastCC:    addLabelToJump(Compiler::getCodeLines()[codeIndex]._vasm, Compiler::getNextInternalLabel()._name); break;
    ...
    bool WHILE(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        Compiler::getWhileWendDataStack().push({0, Compiler::getNextInternalLabel()._name, codeLineIndex, Expression::BooleanCC});
        ...
        if(Compiler::parseExpression(codeLineIndex, conditionToken, condition) == Expression::IsInvalid)
        {
            Cpu::reportError(Cpu::KwdError, stderr, "Keywords::WHILE() : '%s:%d' : syntax error in : '%s' : %s\n", codeLine._moduleName.c_str(), codeLineStart, conditionToken.c_str(), codeLine._text.c_str());
            return false;
        }
        if(condition._ccType == Expression::BooleanCC) Compiler::emitVcpuAsm("%JumpFalse", "", false); // Boolean condition requires this extra check
    ...
    bool WEND(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        switch(whileWendData._ccType)
        {
            case Expression::BooleanCC:
            {
                std::string label = Compiler::getNextInternalLabel()._name;
                if(Compiler::getCodeRomType() < Cpu::ROMvX0  ||  Compiler::getCodeRomType() >= Cpu::SDCARD) label += " " + std::to_string(Compiler::getNextJumpFalseUniqueId());
                addLabelToBooleanCC(Compiler::getCodeLines()[whileWendData._codeLineIndex]._vasm[whileWendData._jmpIndex], "%JumpFalse", label);
            }
            break;
            ...
            case Expression::NormalCC:
            {
                std::string label = Compiler::getNextInternalLabel()._name;
                if(Compiler::getCodeRomType() < Cpu::ROMvX0  ||  Compiler::getCodeRomType() >= Cpu::SDCARD) label += " " + std::to_string(Compiler::getNextJumpFalseUniqueId());
                addLabelToJumpCC(Compiler::getCodeLines()[whileWendData._codeLineIndex]._vasm, label);
            }
            break;

            case Expression::FastCC:
            {
                addLabelToJumpCC(Compiler::getCodeLines()[whileWendData._codeLineIndex]._vasm, Compiler::getNextInternalLabel()._name);
            }
            break;
    ...
    bool REPEAT(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        Compiler::getRepeatUntilDataStack().push({Compiler::getNextInternalLabel()._name, codeLineIndex});
    ...
    bool UNTIL(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        std::string label = repeatUntilData._labelName;
        switch(condition._ccType)
        {
            case Expression::BooleanCC:
            {
                if(Compiler::getCodeRomType() < Cpu::ROMvX0  ||  Compiler::getCodeRomType() >= Cpu::SDCARD) label += " " + std::to_string(Compiler::getNextJumpFalseUniqueId());
                Compiler::emitVcpuAsm("%JumpFalse", label, false);
            }
            break;
            ...
            {
                if(Compiler::getCodeRomType() < Cpu::ROMvX0  ||  Compiler::getCodeRomType() >= Cpu::SDCARD) label += " " + std::to_string(Compiler::getNextJumpFalseUniqueId());
                addLabelToJumpCC(Compiler::getCodeLines()[codeLineIndex]._vasm, label);
            }
            break;
    ...
    bool PSET(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD)
        {
            ...
            if(tokens.size() == 3)
            {
                ...
                Compiler::emitVcpuAsm("STPX", "giga_sysArg0, giga_vAC", false);
                return true;
            }
            else
            {
                Compiler::emitVcpuAsm("STPX", "giga_sysArg0, fgbgColour + 1", false);
                return true;
            }
        }
        else
    ...
    bool DOKE(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD)
        {
            ...
            else if(operandTypes[0] == Compiler::OperandConst  &&  (operandTypes[1] == Compiler::OperandVar  ||  operandTypes[1] == Compiler::OperandTemp))
            {
                ...
                operand = pokeOperandHelper(operandTypes[1], numerics[1], operands[1]);
                (Expression::stringToU16(operands[0], addr)  &&  addr < 0x0100) ? Compiler::emitVcpuAsm("LDI", operands[0], false) : Compiler::emitVcpuAsm("LDWI", operands[0], false);
                Compiler::emitVcpuAsm("DOKEA", "_" + operand, false);
            }
            else
            {
                // Optimise for page 0
    ...
    {
        Cpu::reportError(Cpu::KwdError, stderr, "Keywords::INIT() : '%s:%d' : syntax error, use 'INIT VARS, <optional var address>, TIME, MIDI/MIDIV, <user proc>, NOUPDATE : %s\n",
                                                codeLine._moduleName.c_str(), codeLineStart, codeLine._text.c_str());
    }
    bool INIT(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        if(resetVars)
        {
            if(varsAddr.size())
            {
                ...
                {
                    Cpu::reportError(Cpu::KwdError, stderr, "Keywords::INIT() : '%s:%d' : syntax error in '%s' : %s\n", codeLine._moduleName.c_str(), codeLineStart,
                                                                                                                        Expression::getExpression(), codeLine._text.c_str());
                    return false;
                }
        ...
        if(it != spriteCmdMap.end())
        {
            ...
            switch(spriteCmd)
            {
                ...
                case Compiler::SpriteShow:
                {
                    ...
                    Compiler::emitVcpuAsm("CONDI", "0x80, 0x00", false);
        ...
        if(Compiler::getCodeRomType() >= Cpu::ROMvX0  &&  Compiler::getCodeRomType() < Cpu::SDCARD)
        {
            ...
            if(tokens.size() == 2  &&  stateToken == "OFF")
            {
                ...
                if((expressionType & Expression::HasIntConsts)  &&  constIndex > -1)
                {
                    ...
                    Compiler::emitVcpuAsm("FREQZI", std::to_string(channel - 1), false);
                    return true;
                }
                // Literal channel
                else
    ...
    bool VEC8(Compiler::CodeLine& codeLine, int codeLineIndex, int codeLineStart, int tokenIndex, size_t foundPos, KeywordFuncResult& result)
    {
        ...
        {
            Cpu::reportError(Cpu::KwdError, stderr, "Keywords::VEC8() : '%s:%d' : syntax error, use 'VEC8 ADD, <src addr>, <dst addr>, <size>, <start>' : %s\n", codeLine._moduleName.c_str(), codeLineStart, codeLine._text.c_str());
            ...
            Cpu::reportError(Cpu::KwdError, stderr, "Keywords::VEC8()                           use 'VEC8 CHK, <src addr>, <sub addr>, <CLT>, <CGE>, <size>, <start>'\n");
            return false;
        }
        ...
        switch(vec8Cmd)
        {
            case Compiler::Vec8Add:
            {
                ...
                // Dst
                std::string dstToken = tokens[2];
                Expression::Numeric dstParam;
                if(Compiler::parseExpression(codeLineIndex, dstToken, dstParam) == Expression::IsInvalid)
                {
                    Cpu::reportError(Cpu::KwdError, stderr, "Keywords::VEC8() : '%s:%d' : syntax error in : '%s' : %s\n", codeLine._moduleName.c_str(), codeLineStart, dstToken.c_str(), codeLine._text.c_str());
                    return false;
                ...
                std::string startToken = tokens[4];
