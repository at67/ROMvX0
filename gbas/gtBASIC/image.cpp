...
namespace Image
{
    ...
    bool loadGtRgbFile(const std::string& filename, GtRgbFile& gtRgbFile)
    {
        ...
        if(!infile.is_open())
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadGtRgbFile() : failed to open '%s'\n", filename.c_str());
            return false;
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadGtRgbFile() : couldn't get file size of '%s'\n", filename.c_str());
            return false;
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadGtRgbFile() : bad header in '%s'\n", filename.c_str());
            return false;
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadGtRgbFile() : bad header identifier in '%s'\n", filename.c_str());
            return false;
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadGtRgbFile() : bad header format : %04x : in '%s'\n", header._format, filename.c_str());
            return false;
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadGtRgbFile() : image is larger than 64K bytes : width=%d : height=%d : format=%04x : in '%s'\n", header._width, header._height, header._format, filename.c_str());
            return false;
        }

        if(header._width == 0  ||  header._height == 0)
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadGtRgbFile() : width and height both have to be non zero : width=%d : height=%d : format=%04x : in '%s'\n", header._width, header._height, header._format, filename.c_str());
            return false;
        }
        ...
        if(infile.eof() || infile.bad() || infile.fail())
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadGtRgbFile() : bad data in '%s'\n", filename.c_str());
            return false;
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadGtRgbFile() : bad optional size in '%s'\n", filename.c_str());
            return false;
        }
    ...
    bool saveGtRgbFile(const std::string& filename, GtRgbFile& gtRgbFile)
    {
        ...
        if(!outfile.is_open())
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::saveGtRgbFile() : failed to open '%s'\n", filename.c_str());
            return false;
        }

        if(gtRgbFile._header._format > GtRgbFormats::GT_RGB_888)
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::saveGtRgbFile() : bad header format : %04x : for '%s'\n", gtRgbFile._header._format, filename.c_str());
            return false;
        }
        ...
        if(totalSize >= 0x10000)
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::saveGtRgbFile() : image is larger than 64K bytes : width=%d : height=%d : format=%04x : in '%s'\n",
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::saveGtRgbFile() : width and height both have to be non zero : width=%d : height=%d : format=%04x : in '%s'\n",
                                                    gtRgbFile._header._width, gtRgbFile._header._height, gtRgbFile._header._format, filename.c_str());
            return false;
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::saveGtRgbFile() : image size does not match data size : image size=%d : data size=%d : in '%s'\n", totalSize, int(gtRgbFile._data.size()), filename.c_str());
            return false;
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::saveGtRgbFile() : write error in header of '%s'\n", filename.c_str());
            return false;
        }
        ...
        if(outfile.bad() || outfile.fail())
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::saveGtRgbFile() : write error in data of '%s'\n", filename.c_str());
            return false;
        }
    ...
    bool loadTgaFile(const std::string& filename, TgaFile& tgaFile)
    {
        ...
        if(!infile.is_open())
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : failed to open '%s'\n", filename.c_str());
            return false;
        }
        ...
        if(infile.eof() || infile.bad() || infile.fail())
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : bad header in '%s'\n", filename.c_str());
            return false;
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : bad colourMapType %d, the only valid colourMapType is 0 : in '%s'\n", header._colourMapType, filename.c_str());
            return false;
        }

        if(header._imageType != 2)
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : bad imageType %d, the only valid imageType is 2 : in '%s'\n", header._imageType, filename.c_str());
            return false;
        }

        if(header._colourMapDepth !=0  ||  header._colourMapLength != 0  ||  header._colourMapOrigin != 0)
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : bad colourMap entries, colour maps, (palettes), not supported : in '%s'\n", filename.c_str());
            return false;
        }

        if(header._originX !=0  ||  header._originY != 0)
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : bad origin, origin anything other thant (0, 0), not supported : in '%s'\n", filename.c_str());
            return false;
        }

        if(header._bitsPerPixel != 24  &&  header._bitsPerPixel != 32)
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : bad bitsPerPixel, only bitsPerPixel = 8 is supported : in '%s'\n", filename.c_str());
            return false;
        }

        if(header._width * header._height >= 0x10000)
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : maximum width*height = 64Kbytes, width = %d, height =%d, size = %d : in '%s'\n",
        ...
        if(header._width == 0  ||  header._height == 0)
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : width and height both have to be non zero, width = %d, height =%d : in '%s'\n", header._width, header._height, filename.c_str());
            return false;
        }

        uint8_t attributeBits = header._imageDescriptor & 0x0F;
        if(attributeBits)
        {
            // 32Bit ARGB files must have the attribute bits per pixel, (alpha bits per pixel), set to 8
            if(header._bitsPerPixel != 32  ||  attributeBits != 8)
            {
                Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : invalid alpha/attribute bits %01x for bits per pixel of %d : in '%s'\n", attributeBits, header._bitsPerPixel, filename.c_str());
                return false;
            }
        }
        ...
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : image is larger than %d bytes : width=%d : height=%d : in '%s'\n",
        ...
        if(infile.eof() || infile.bad() || infile.fail())
        {
            Cpu::reportError(Cpu::ImgError, stderr, "Image::loadTgaFile() : bad data in '%s'\n", filename.c_str());
            return false;
        }
    ...
    // *NOTE* TGA and the Gigatron store RGB pixels in little endian order, so BGR in memory
    uint8_t convertArgb32ToArgb8(uint8_t alpha, uint8_t red, uint8_t green, uint8_t blue)
    {
        alpha = convertTo2Bits(alpha);
        red = convertTo2Bits(red);
        green = convertTo2Bits(green);
        blue = convertTo2Bits(blue);
        return uint8_t((alpha & 0xC0) | ((blue & 0xC0) >>2) | ((green & 0xC0) >>4) | ((red & 0xC0) >>6));
    }
    ...
    bool convertARGB8toRGB2(const std::vector<uint8_t>& src, std::vector<uint8_t>& dst, int width, int height, uint8_t imageOrigin)
    {
    ...
    // 32bit to 8bit
    bool convertARGB8toARGB2(const std::vector<uint8_t>& src, std::vector<uint8_t>& dst, int width, int height, uint8_t imageOrigin)
    {
        if(int(src.size()) != width*height*4) return false;
        dst.resize(width*height);
        ...
        for(int y=startY; y!=endY; y+=stepY)
        {
            for(int x=startX; x!=endX; x+=stepX)
            {
                int indexRGB2 = getPixelAddress(width, x, y);
                int indexRGBA8 = indexRGB2 * 4;
                *ptr++ = convertArgb32ToArgb8(src[indexRGBA8 + 3], src[indexRGBA8 + 2], src[indexRGBA8 + 1], src[indexRGBA8 + 0]);
            }
        }
    ...
    bool ditherARGB8toRGB2(std::vector<uint8_t>& src, std::vector<uint8_t>& dst, int width, int height, uint8_t imageOrigin)
    {
        if(int(src.size()) != width*height*4) return false;
        dst.resize(width*height);
...
//**********************************************************************************************************************
    ...
    void handleMouseLeftButtonDown(int mouseX, int mouseY)
    {
        ...
        Cpu::reportError(Cpu::NoError, stderr, "%d %d %f %f %d %d\n", mouseX, mouseY, mx, my, pixelX, pixelY);
    }
    ...
    void handleMouseButtonUp(const SDL_Event& event, const Editor::MouseState& mouseState)
    {
        ...
        switch(menuItemIndex)
        {
            ...
            case MenuLoad:
            {
                Cpu::reportError(Cpu::NoError, stderr, "Image::Load()\n");
            }
            break;
            ...
            case MenuSave:
            {
                Cpu::reportError(Cpu::NoError, stderr, "Image::Save()\n");
            }
            break;
            ...
            case MenuSel:
            {
                Cpu::reportError(Cpu::NoError, stderr, "Image::Select()\n");
            }
            break;
            ...
            case MenuDel:
            {
                Cpu::reportError(Cpu::NoError, stderr, "Image::Delete()\n");
            }
            break;
            ...
            case MenuErase:
            {
                Cpu::reportError(Cpu::NoError, stderr, "Image::Erase()\n");
            }
            break;
    ...
    void handleKey(const SDL_Event& event)
    {
        ...
        Cpu::reportError(Cpu::NoError, stderr, "%c", keyCode);
    }
    ...
    void handleKeyDown(SDL_Keycode keyCode, Uint16 keyMod)
    {
        ...
        // VcpuTrace
        else if(keyCode == Editor::getEmulatorScanCode("VcpuTrace")  &&  keyMod == Editor::getEmulatorKeyMod("VcpuTrace"))
        {
            _firstTimeRender = true;
